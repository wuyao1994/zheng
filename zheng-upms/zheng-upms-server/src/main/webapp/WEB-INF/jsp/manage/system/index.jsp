...
function actionFormatter(value, row, index) {
    return [
        '<a class="update" href="javascript:;" onclick="updateAction('+row.systemId+')" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>　',
        '<a class="delete" href="javascript:;" onclick="deleteAction('+row.systemId+')" data-toggle="tooltip" title="Remove"><i class="glyphicon glyphicon-remove"></i></a>'
    ].join('');
}
// 格式化图标
...
function updateAction(systemId) {
	...
	} else if (rows.length != 1 && systemId == null) {
		$.confirm({
			title: false,
			content: '请选择一条记录！',
...
function deleteAction(systemId) {
	...
	} else if (rows.length == 0 && systemId == null) {
		$.confirm({
			title: false,
			content: '请至少选择一条记录！',
			autoClose: 'cancel|3000',
