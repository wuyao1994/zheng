...
function actionFormatter(value, row, index) {
    return [
        '<a class="update" href="javascript:;" onclick="updateAction('+row.pageId+')" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>　',
        '<a class="delete" href="javascript:;" onclick="deleteAction('+row.pageId+')" data-toggle="tooltip" title="Remove"><i class="glyphicon glyphicon-remove"></i></a>'
    ].join('');
}
// 格式化时间
...
function updateAction(pageId) {
	...
	if (pageId != null) {
        updateDialog = $.dialog({
            animationSpeed: 300,
            title: '编辑单页',
            content: 'url:${basePath}/manage/page/update/' + pageId,
            onContentReady: function () {
                initMaterialInput();
            }
        });
	} else if (rows.length != 1 && pageId == null) {
		$.confirm({
			title: false,
			content: '请选择一条记录！',
...
function deleteAction(pageId) {
	...
	} else if (rows.length == 0 && pageId == null) {
		$.confirm({
			title: false,
			content: '请至少选择一条记录！',
			autoClose: 'cancel|3000',
