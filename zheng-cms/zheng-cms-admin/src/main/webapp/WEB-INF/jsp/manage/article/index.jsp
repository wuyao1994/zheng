...
function actionFormatter(value, row, index) {
    return [
        '<a class="update" href="javascript:;" onclick="updateAction('+row.articleId+')" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-edit"></i></a>　',
        '<a class="delete" href="javascript:;" onclick="deleteAction('+row.articleId+')" data-toggle="tooltip" title="Remove"><i class="glyphicon glyphicon-remove"></i></a>'
    ].join('');
}
// 格式化类型
...
// 编辑
var updateDialog;
function updateAction(articleId) {
	...
	if (articleId != null) {
        updateDialog = $.dialog({
            ...
            title: '编辑文章',
            columnClass: 'xlarge',
            content: 'url:${basePath}/manage/article/update/' + articleId,
            onContentReady: function () {
                initMaterialInput();
                $('select').select2();
            }
        });
	} else if (rows.length != 1 && articleId == null) {
		$.confirm({
			title: false,
			content: '请选择一条记录！',
...
function deleteAction(articleId) {
	...
	} else if (rows.length == 0 && articleId == null) {
		$.confirm({
			title: false,
			content: '请至少选择一条记录！',
			autoClose: 'cancel|3000',
